...
<a href="https://drive.google.com/file/d/15C5YonFTLUoSKdL2sHehB0lBsNEsc9Zn/view?usp=drive_link" target="_blank">https://drive.google.com/file/d/15C5YonFTLUoSKdL2sHehB0lBsNEsc9Zn/view?usp=drive_link</a>
...
- [DeliverEnergy](#deliverenergy)
  ...
  - [4. ゲーム説明](#4-ゲーム説明)
    ...
    - [2. 眷属](#2-眷属)
    - [3. 雑魚敵](#3-雑魚敵)
    - [4.4 リザルト画面について](#44-リザルト画面について)
    - [リザルト画面](#リザルト画面)
    - [4.5 設定画面について](#45-設定画面について)
    - [設定画面](#設定画面)
    - [4.6 ムービーについて](#46-ムービーについて)
    - [1. オープニングムービー](#1-オープニングムービー)
    - [オープニングムービー](#オープニングムービー)
    - [2. ボス登場ムービー](#2-ボス登場ムービー)
    - [ボス登場ムービー](#ボス登場ムービー)
    - [3. ボス撃破ムービー　ーこだわりポイント！！ー](#3-ボス撃破ムービーーこだわりポイントー)
    - [ボス撃破ムービー](#ボス撃破ムービー)
    - [4 カメラの座標出力について](#4-カメラの座標出力について)
    - [デバックカメラ](#デバックカメラ)
    - [実際に出力された座標](#実際に出力された座標)
    - [4.7 コインについて](#47-コインについて)
    - [コイン](#コイン)
  - [5. 技術紹介](#5-技術紹介)
    - [1. プレイヤーの転がり処理](#1-プレイヤーの転がり処理)
    - [2. 円形のシェーダー](#2-円形のシェーダー)
...
### 4.4 リザルト画面について
ゴールにたどり着くとスコアが表示されます。
ただ表示するだけではなく**カウントアップ形式**でスコアを表示させることにより、視覚的に楽しめるようにしました。

### リザルト画面
![リザルト画面](Md_Gif/Result.gif)


### 4.5 設定画面について
**タイトル画面**と**ゲーム中**に設定画面で音量調節が出来ます。

**全体音量**・**BGM音量**・**SE音量**をそれぞれ調節可能です。

### 設定画面
![設定画面](Md_Gif/Setting.gif)

### 4.6 ムービーについて
目で楽しめむために、様々な場面で**ムービー**を用意しました。

### 1. オープニングムービー
カウントダウンが終了すると、**オープニングムービー**が流れます。
このムービーはプレイヤーがこれからどこに行くのかを明確にするための目印として導入しました。

### オープニングムービー
※長いので倍速にしています。
![オープニングムービー](Md_Gif/EventCamera.gif)

### 2. ボス登場ムービー
ボスが出現するゴーストオブジェクトに触れると、**ボス登場ムービー**が流れます。
ボスの声と相まってよりバグらしさを演出しました。

### ボス登場ムービー
![ボス登場ムービー](Md_Gif/Boss_syutugen.gif)

### 3. ボス撃破ムービー　ーこだわりポイント！！ー
ボスのHPを削りきると**ボス撃破ムービー**が流れます。
エフェクトをふんだんに使ったり、最後にカメラシェイクをすることで爆発の前触れを作り、迫力のある撃破シーンを演出できました。

### ボス撃破ムービー
![ボス撃破ムービー](Md_Gif/BossDie.gif)

### 4 カメラの座標出力について
このムービーはカメラのイージング処理を行って座標間を移動することでムービーを作っています。
その際に座標を抽出する際にモデル作成ソフトとして使っている3dsMaxを使用していましたが、プログラム側で手打ちで座標を入力する手間が発生していました。
そこでDebugCameraクラスを作りました。
このカメラクラスでは、プレイヤーを追従するカメラとは別のカメラを作り、Aボタンを入力すると「カメラ本体の座標・カメラが向いている方向」を出力タブで出力できるようにしました。

### デバックカメラ
![デバッグカメラ](Md_Gif/DebugCamera.gif)

### 実際に出力された座標
![座標](Md_Sprite/Debug.png)


### 4.7 コインについて
ボスを倒した後に**コイン**を出現させました。
最終的に合計スコアに差がつく部分で、スコアアタック要素として実装しました。
モデルのポリゴンをところどころ消去してデジタル感を出しました。
...
### 2. 円形のシェーダー
* **課題**: 使用しているエンジンには画像の『減少・増加』といった変動処理が、仕様上『上下左右』の直線的な動きにしか対応していませんでした。
しかし、今回ゲームのデザイン的にどうしても「円形」のゲージを採用したいと考えていました。

* **工夫**: 
    #### 1.円形シェーダーの実装:
     円形に切り取る処理（クリップ処理）を実装しています。これにより、エンジンの標準機能にはなかった『円形の滑らかな増減』を実現することができました。
     
     UV座標からatan2(アークタンジェント２)関数を用いて角度を算出し、HPの現在値に合わせて描画ピクセルを動的にclip（破棄）するロジックを組んでいます。
     
* **結果**: 円形の滑らかな増減を実現することができました。
  ![HPバー](Md_Gif/HP.gif)
